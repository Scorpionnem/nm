...
int	parse_elf_header(t_ctx *ctx)
{
	...
	if (ctx->map.size < sizeof(Elf64_Ehdr))
		return (printf("ft_nm: %s: file too small\n", ctx->path), 0);
	...
	if (type != ET_REL && type != ET_EXEC && type != ET_DYN)
		return (printf("ft_nm: %s: invalid object type %d\n", ctx->path, type), -1);
	...
	if ((!ctx->is_x64 && machine != EM_386) || (ctx->is_x64 && machine != EM_X86_64))
		return (printf("ft_nm: %s: invalid architecture %d\n", ctx->path, machine), -1);
